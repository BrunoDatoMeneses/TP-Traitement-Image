...
unsigned char* erosion (unsigned char* image, int width, int height)
{
    ...
    for (h = 0; h < (int)height; h++)
    {
        for (w = 0; w < (int)width; w++)
        {
            ...
            {
                temp[(h * width) + w]=255; /* image blanche */
                ...
                for (dx =-size; dx <= size; dx++)
                {
                    ...
                    {
                        /* si un pixel noir dans le noyau autour du pixel (h*width) + w 
                        dans image, alors ce pixel sera noir sur temp */
